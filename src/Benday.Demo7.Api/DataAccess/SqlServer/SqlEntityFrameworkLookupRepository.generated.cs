...
namespace Benday.Demo7.Api.DataAccess.SqlServer
{
    public partial class SqlEntityFrameworkLookupRepository :
        SqlEntityFrameworkSearchableRepositoryBase<LookupEntity, Demo7DbContext>,
        ILookupRepository
    {
        public SqlEntityFrameworkLookupRepository(
            Demo7DbContext context) : base(context)
        {
        }
            ...
            SearchArgument arg)
        {
            ...
            {
                if (arg.PropertyName == nameof(LookupEntity.DisplayOrder) && arg.Method != SearchMethod.Skip && int.TryParse(arg.SearchValue, out _) == true)
                {
                    return q => q.DisplayOrder == int.Parse(arg.SearchValue);
                }
                else if (arg.PropertyName == nameof(LookupEntity.LookupType))
                {
                ...
                else if (arg.PropertyName == nameof(LookupEntity.Id) && arg.Method != SearchMethod.Skip && int.TryParse(arg.SearchValue, out _) == true)
                {
                    return q => q.Id == int.Parse(arg.SearchValue);
                }
                else if (arg.PropertyName == nameof(LookupEntity.Status))
                {
        ...
        {
            if (string.Compare(propertyName, "DisplayOrder", true) == 0)
            {
                if (isFirstSort == true)
                {
            ...
            else if (string.Compare(propertyName, "Id", true) == 0)
            {
                if (isFirstSort == true)
                {
