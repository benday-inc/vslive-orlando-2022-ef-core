...
namespace Benday.Demo7.Api.DataAccess.SqlServer
{
    public partial class SqlEntityFrameworkLogEntryRepository :
        SqlEntityFrameworkSearchableRepositoryBase<LogEntryEntity, Demo7DbContext>,
        ILogEntryRepository
    {
            ...
            SearchArgument arg)
        {
            ...
            {
                if (arg.PropertyName == nameof(LogEntryEntity.Id) && arg.Method != SearchMethod.Skip && int.TryParse(arg.SearchValue, out _) == true)
                {
                    return q => q.Id == int.Parse(arg.SearchValue);
                }
                else if (arg.PropertyName == nameof(LogEntryEntity.Category))
                {
