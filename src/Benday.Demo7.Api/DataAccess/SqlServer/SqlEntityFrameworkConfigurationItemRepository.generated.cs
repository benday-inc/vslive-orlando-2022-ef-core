...
namespace Benday.Demo7.Api.DataAccess.SqlServer
{
    public partial class SqlEntityFrameworkConfigurationItemRepository :
        SqlEntityFrameworkSearchableRepositoryBase<ConfigurationItemEntity, Demo7DbContext>,
        IConfigurationItemRepository
    {
            ...
            SearchArgument arg)
        {
            ...
            else
            {
                ...
                else if (arg.PropertyName == nameof(ConfigurationItemEntity.Id) && arg.Method != SearchMethod.Skip && int.TryParse(arg.SearchValue, out _) == true)
                {
        ...
        {
            if (string.Compare(propertyName, "Category", true) == 0)
            {
                if (isFirstSort == true)
                {
