...
namespace Benday.Demo7.Api.DataAccess.SqlServer
{
    public partial class SqlEntityFrameworkFeedbackRepository :
        SqlEntityFrameworkSearchableRepositoryBase<FeedbackEntity, Demo7DbContext>,
        IFeedbackRepository
    {
            ...
            SearchArgument arg)
        {
            ...
            else
            {
                ...
                else if (arg.PropertyName == nameof(FeedbackEntity.Id) && arg.Method != SearchMethod.Skip && int.TryParse(arg.SearchValue, out _) == true)
                {
        ...
        {
            if (string.Compare(propertyName, "FeedbackType", true) == 0)
            {
                if (isFirstSort == true)
                {
