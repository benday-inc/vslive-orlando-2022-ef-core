...
describe('LookupSearchComponent', () => {
  let systemUnderTest: LookupSearchComponent;
  ...
  it('should populate list of lookup items on successful search', () => {
    // arrange
    const service = fixture.debugElement.injector.get(LookupService);
    ...
    const results = new SimpleSearchResults<Lookup>();
  ...
  it('reset() should set search to default and call search() with empty search value', () => {
    ...
    serviceSearchMethodSpy.and.returnValue(
      of(new SimpleSearchResults<Lookup>())
    );
  ...
  it('should set message to no data when there is no data', () => {
    ...
    const serviceMethodSpy = spyOn(service, 'search');
    serviceMethodSpy.and.returnValue(of(new SimpleSearchResults<Lookup>()));
