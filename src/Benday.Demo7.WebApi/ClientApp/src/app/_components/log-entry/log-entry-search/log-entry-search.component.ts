...
@Component({
  selector: 'app-log-entry-search',
  templateUrl: './log-entry-search.component.html',
  styleUrls: ['./log-entry-search.component.css'],
})
export class LogEntrySearchComponent implements OnInit {
  public searchValue: string = ApplicationConstants.emptySearchValue;
  public items: SimpleSearchResults<LogEntry> =
    new SimpleSearchResults<LogEntry>();
  @Input() public autoLoad = true;
  @Input() public hideCreate = false;
  public message: string = ApplicationConstants.emptyMessage;

  constructor(private service: LogEntryService, private router: Router) {}
  ...
  load() {
    if (this.autoLoad === true) {
      ...
      this.items = new SimpleSearchResults<LogEntry>();
      this.message = ApplicationConstants.emptyMessage;
    }
  }
  ...
  private handleSearchResults(data: SimpleSearchResults<LogEntry>) {
    this.items = data;
