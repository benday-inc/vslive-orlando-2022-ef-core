...
describe('FeedbackSearchComponent', () => {
  let systemUnderTest: FeedbackSearchComponent;
  ...
  it('should populate list of feedback items on successful search', () => {
    // arrange
    const service = fixture.debugElement.injector.get(FeedbackService);
    ...
    const results = new SimpleSearchResults<Feedback>();
  ...
  it('reset() should set search to default and call search() with empty search value', () => {
    ...
    serviceSearchMethodSpy.and.returnValue(
      of(new SimpleSearchResults<Feedback>())
    );
  ...
  it('should set message to no data when there is no data', () => {
    ...
    const serviceMethodSpy = spyOn(service, 'search');
    serviceMethodSpy.and.returnValue(of(new SimpleSearchResults<Feedback>()));
